...
const colors = ['#2dd4bf', '#60a5fa', '#facc15', '#fb7185', '#a78bfa', '#34d399', '#f97316', '#c084fc']
const gridLines = [42, 86, 130, 174, 218]
const b3FocusAssets = ['DI1', 'DDI', 'DOL', 'WDO', 'WIN']
const b3AssetTabs = ['ALL', ...b3FocusAssets]
const b3MonthOrder = Object.fromEntries([...('FGHJKMNQUVXZ')].map((code, index) => [code, index + 1]))
const FUNDS_FLOW_HISTORY_DAYS = 30
const nportTargets = [
...
const cdaGraphTargets = [
  { key: 'all', label: 'Todos' },
  ...cdaTargets,
]
const moneyFlowModes = [
...
function flowHeatColor(value, maxAbs) {
  const parsed = Number(value)
  if (!Number.isFinite(parsed)) return 'rgba(51, 65, 85, 0.35)'
...
function nportDivergingColor(value, maxAbs = 100) {
  const parsed = Number(value)
...
function nportCellTint(value, maxAbs) {
  const parsed = Number(value)
...
function nportRowTint(value, maxAbs) {
  const parsed = Number(value)
...
function nportTileBackground(value, strength) {
...
function nportCountryPillStyle(country) {
...
function totalPages(payloadLike) {
...
function portfolioSharedFactorText(item = {}) {
  const direct = [
...
function cdaScatterTitle(point) {
...
function nportScatterTitle(point) {
...
function nportCountryOrbitTitle(point) {
...
function iciHeatTitle(row, cell) {
