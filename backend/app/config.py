...
DEFAULT_MACRO_BLOOMBERG_REFERENCE_ASSETS = [
    ...
    {
        "security": "XB1 Index",
        "label": "Bovespa Index Future",
        "category": "equity",
        "bucket": "equity",
    },
    {
        "security": "ESA Index",
        "label": "S&P 500 Future Proxy",
        "category": "equity",
        "bucket": "equity",
    },
    {
        "security": "MES1 Index",
        "label": "Emerging Markets Future",
        "category": "equity",
        "bucket": "equity",
    },
    {
        "security": "DMA Index",
        "label": "Developed Markets Basket",
        "category": "equity",
        "bucket": "equity",
    },
    {"security": "RTYA Index", "label": "Russell 2000", "category": "equity", "bucket": "equity"},
    {
        "security": "EMHY CDSI S44 5Y PRC Corp",
        "label": "EM HY CDSI S44 5Y",
        "category": "credit",
        "bucket": "credit",
    },
    {
        "security": "EMBIV Index",
        "label": "EMBI Volatility",
        "category": "credit",
        "bucket": "credit",
    },
    {
        "security": "CDX HY CDSI GEN 5Y SPRD Corp",
        "label": "CDX HY CDSI",
        "category": "credit",
        "bucket": "credit",
    },
    {
        "security": "BRAZIL CDS USD SR 3Y D14 Curncy",
        "label": "Brazil CDS USD",
        "category": "sovereign",
        "bucket": "credit",
    },
    {"security": ".JPYB U Index", "label": "JPY Basket", "category": "fx", "bucket": "fx"},
    {
        "security": "CDX EM CDSI S44 5Y PRC Corp",
        "label": "CDX EM CDSI S44",
        "category": "credit",
        "bucket": "credit",
    },
]
...
def _parse_key_value_mapping(
    raw_value: str, item_separator: str = ",", kv_separator: str = "="
) -> dict[str, str]:
...
DEFAULT_OPTIONS_MODEL_FALLBACK_RATE = _parse_float(
    os.environ.get("OPTIONS_MODEL_FALLBACK_RATE"), 0.135
)
...
class Config:
    ...
    RAW_STORAGE_ROOT = os.environ.get(
        "AQUILES_RAW_STORAGE_ROOT",
        str(PROJECT_ROOT / "backend" / "uploads"),
    )
    SECRET_KEY = os.environ.get("SECRET_KEY") or secrets.token_hex(32)
    DEBUG = os.environ.get("FLASK_DEBUG", "False").lower() == "true"
    COLLECTION_SCHEDULER_SERVICE_HOST = os.environ.get(
        "COLLECTION_SCHEDULER_SERVICE_HOST",
        "127.0.0.1",
    )
    COLLECTION_SCHEDULER_SERVICE_PORT = int(
        os.environ.get("COLLECTION_SCHEDULER_SERVICE_PORT", "5023")
    )
    COLLECTION_SCHEDULER_SERVICE_URL = os.environ.get(
        "COLLECTION_SCHEDULER_SERVICE_URL",
        "http://127.0.0.1:5023",
    ).rstrip("/")
    COLLECTION_SCHEDULER_REQUEST_TIMEOUT_SECONDS = float(
        os.environ.get("COLLECTION_SCHEDULER_REQUEST_TIMEOUT_SECONDS", "60")
    )
    INTERNAL_SERVICE_TOKEN = os.environ.get(
        "AQUILES_INTERNAL_SERVICE_TOKEN",
    ) or os.environ.get("AQUILES_AUTH_TOKEN_SECRET", "")
    CORS_ORIGINS = [
        origin.strip()
    ...
    CVM_CDA_GRAPH_MAX_POSITIONS_PER_FUND = int(
        os.environ.get("CVM_CDA_GRAPH_MAX_POSITIONS_PER_FUND", "30")
    )
    CVM_CDA_GRAPH_MIN_ABS_VALUE = float(os.environ.get("CVM_CDA_GRAPH_MIN_ABS_VALUE", "10000000"))
    CVM_CDA_GRAPH_TARGET_FUNDS_PER_THEME = int(
        os.environ.get("CVM_CDA_GRAPH_TARGET_FUNDS_PER_THEME", "60")
    )
    ...
    MACRO_INGEST_AUTO_RESTART = (
        os.environ.get("MACRO_INGEST_AUTO_RESTART", "True").lower() == "true"
    )
    ...
    MACRO_BLEU_RECONNECT_DELAY_SECONDS = int(
        os.environ.get("MACRO_BLEU_RECONNECT_DELAY_SECONDS", "2")
    )
    MACRO_INDEX_TICKERS = [
        ticker.strip()
    ...
    MACRO_CURVE_LONG_TICKERS = [
        ...
        for ticker in os.environ.get(
            "MACRO_CURVE_LONG_TICKERS", "BVMF:DI1F29,BVMF:DI1F30,BVMF:DI1F31,BVMF:DI1F35"
        ).split(",")
        if ticker.strip()
    ]
    ...
    MACRO_AQUANT_SECURITY_SYMBOLS = [
        ...
        for symbol in os.environ.get(
            "MACRO_AQUANT_SECURITY_SYMBOLS", "VALE3,PETR4,ITUB4,BPAC11,BBDC4"
        ).split(",")
        if symbol.strip()
    ]
    MACRO_AQUANT_OHLCV_INTERVAL = os.environ.get("MACRO_AQUANT_OHLCV_INTERVAL", "1 min")
    MACRO_AQUANT_OHLCV_WINDOW_MINUTES = int(
        os.environ.get("MACRO_AQUANT_OHLCV_WINDOW_MINUTES", "30")
    )
    ...
    MACRO_PARTICIPANT_HEATMAP_INTERVAL_SECONDS = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_INTERVAL_SECONDS", "15")
    )
    MACRO_PARTICIPANT_HEATMAP_HISTORY_MINUTES = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_HISTORY_MINUTES", "45")
    )
    MACRO_PARTICIPANT_HEATMAP_PARTICIPANT_LIMIT = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_PARTICIPANT_LIMIT", "14")
    )
    MACRO_PARTICIPANT_HEATMAP_CATALOG_LIMIT = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_CATALOG_LIMIT", "10")
    )
    MACRO_PARTICIPANT_HEATMAP_CANDLE_LIMIT = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_CANDLE_LIMIT", "45")
    )
    MACRO_PARTICIPANT_HEATMAP_DAY_CANDLE_LIMIT = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_DAY_CANDLE_LIMIT", "720")
    )
    MACRO_PARTICIPANT_HEATMAP_SESSION_START_HOUR = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_SESSION_START_HOUR", "9")
    )
    MACRO_PARTICIPANT_HEATMAP_SESSION_START_MINUTE = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_SESSION_START_MINUTE", "0")
    )
    MACRO_PARTICIPANT_HEATMAP_ENABLE = (
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_ENABLE", "True").lower() == "true"
    )
    MACRO_PARTICIPANT_HEATMAP_AUTO_START = (
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_AUTO_START", "True").lower() == "true"
    )
    MACRO_PARTICIPANT_HEATMAP_SESSION_SAMPLE_LIMIT = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_SESSION_SAMPLE_LIMIT", "2600")
    )
    MACRO_PARTICIPANT_HEATMAP_RENDER_LOOKBACK_MINUTES = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_RENDER_LOOKBACK_MINUTES", "20")
    )
    MACRO_PARTICIPANT_HEATMAP_POINT_LIMIT = int(
        os.environ.get("MACRO_PARTICIPANT_HEATMAP_POINT_LIMIT", "420")
    )
    MACRO_PARTICIPANT_PRESSURE_WINDOWS = _parse_int_list(
        os.environ.get("MACRO_PARTICIPANT_PRESSURE_WINDOWS", "1,3,5,15"),
        [1, 3, 5, 15],
    )
    MACRO_PARTICIPANT_PRESSURE_PRIMARY_WINDOW = int(
        os.environ.get("MACRO_PARTICIPANT_PRESSURE_PRIMARY_WINDOW", "5")
    )
    MACRO_PARTICIPANT_VALUE_AREA_RATIO = float(
        os.environ.get("MACRO_PARTICIPANT_VALUE_AREA_RATIO", "0.70")
    )
    MACRO_PARTICIPANT_VALUE_MAX_LEVELS = int(
        os.environ.get("MACRO_PARTICIPANT_VALUE_MAX_LEVELS", "8")
    )
    MACRO_NEWS_LINK_LOOKBACK_MINUTES = int(
        os.environ.get("MACRO_NEWS_LINK_LOOKBACK_MINUTES", "180")
    )
    ...
    MACRO_OPTIONS_GAMMA_REGION_LIMIT = int(os.environ.get("MACRO_OPTIONS_GAMMA_REGION_LIMIT", "14"))
    MACRO_BLOOMBERG_ENABLE = os.environ.get("MACRO_BLOOMBERG_ENABLE", "True").lower() == "true"
    BLOOMBERG_REALTIME_REFERENCE_ENABLE = (
        os.environ.get("BLOOMBERG_REALTIME_REFERENCE_ENABLE", "False").lower() == "true"
    )
    BLOOMBERG_EXCEL_FALLBACK_ENABLE = (
        os.environ.get("BLOOMBERG_EXCEL_FALLBACK_ENABLE", "False").lower() == "true"
    )
    ...
    MACRO_THERMOMETER_ENABLE_LLM = (
        os.environ.get("MACRO_THERMOMETER_ENABLE_LLM", "False").lower() == "true"
    )
    MACRO_REPORT_SOURCES_ENABLE = (
        os.environ.get("MACRO_REPORT_SOURCES_ENABLE", "True").lower() == "true"
    )
    MACRO_REPORT_SOURCES_AUTO_START = (
        os.environ.get("MACRO_REPORT_SOURCES_AUTO_START", "True").lower() == "true"
    )
    MACRO_REPORT_SOURCES_UPDATE_TIME = os.environ.get(
        "MACRO_REPORT_SOURCES_UPDATE_TIME", "07:30"
    ).strip()
    MACRO_REPORT_SOURCES_LOOKBACK_DAYS = int(
        os.environ.get("MACRO_REPORT_SOURCES_LOOKBACK_DAYS", "30")
    )
    MACRO_REPORT_SOURCES_TIMEOUT_SECONDS = float(
        os.environ.get("MACRO_REPORT_SOURCES_TIMEOUT_SECONDS", "12")
    )
    MACRO_REPORT_SOURCES_CACHE_SECONDS = int(
        os.environ.get("MACRO_REPORT_SOURCES_CACHE_SECONDS", "900")
    )
    ...
    FUNDS_FLOW_LOCAL_AUTO_START = (
        os.environ.get("FUNDS_FLOW_LOCAL_AUTO_START", "False").lower() == "true"
    )
    ...
    FUNDS_FLOW_LOCAL_TIMEOUT_SECONDS = float(
        os.environ.get("FUNDS_FLOW_LOCAL_TIMEOUT_SECONDS", "45")
    )
    ...
    ETF_DAILY_FLOW_AUTO_START = (
        os.environ.get("ETF_DAILY_FLOW_AUTO_START", "True").lower() == "true"
    )
    ...
    ETF_DAILY_FLOW_CAPTURE_TIMES = os.environ.get(
        "ETF_DAILY_FLOW_CAPTURE_TIMES", "20:15,22:15,00:30"
    )
    ...
    ETF_DAILY_FLOW_SCHEDULER_POLL_SECONDS = int(
        os.environ.get("ETF_DAILY_FLOW_SCHEDULER_POLL_SECONDS", "60")
    )
    ETF_DAILY_FLOW_REQUEST_TIMEOUT_SECONDS = float(
        os.environ.get("ETF_DAILY_FLOW_REQUEST_TIMEOUT_SECONDS", "25")
    )
    ETF_DAILY_FLOW_REQUEST_MAX_ATTEMPTS = int(
        os.environ.get("ETF_DAILY_FLOW_REQUEST_MAX_ATTEMPTS", "3")
    )
    ETF_DAILY_FLOW_RETRY_BACKOFF_SECONDS = float(
        os.environ.get("ETF_DAILY_FLOW_RETRY_BACKOFF_SECONDS", "3")
    )
    ETF_DAILY_FLOW_MAX_STALE_HOURS = float(os.environ.get("ETF_DAILY_FLOW_MAX_STALE_HOURS", "48"))
    ETF_DAILY_FLOW_CONTRACT_FAILURE_THRESHOLD = int(
        os.environ.get("ETF_DAILY_FLOW_CONTRACT_FAILURE_THRESHOLD", "2")
    )
    ...
    OPLAB_CHAIN_STABILITY_POLL_SECONDS = float(
        os.environ.get("OPLAB_CHAIN_STABILITY_POLL_SECONDS", "0.5")
    )
    ...
    OPLAB_UNDERLYING_MAP = {
        ...
        for item in os.environ.get(
            "OPLAB_UNDERLYING_MAP", "IBOVE Index=IBOV,BOVA11 Index=BOVA11"
        ).split(",")
    ...
    OPTIONS_INGEST_AUTO_RESTART = (
        os.environ.get("OPTIONS_INGEST_AUTO_RESTART", "True").lower() == "true"
    )
    ...
    OPTIONS_BLOOMBERG_UNDERLYINGS = [
        security.strip()
        for security in os.environ.get(
            "OPTIONS_BLOOMBERG_UNDERLYINGS", "IBOVE Index,SPX Index"
        ).split(",")
        if security.strip()
    ]
    OPTIONS_UNDERLYING_TRADE_MAP = _parse_key_value_mapping(
        os.environ.get(
            "OPTIONS_UNDERLYING_TRADE_MAP", "IBOVE Index=BVMF:WINM26,SPX Index=ESA Index"
        )
    )
    ...
    OPTIONS_TICK_CAPTURE_ENABLE = (
        os.environ.get("OPTIONS_TICK_CAPTURE_ENABLE", "False").lower() == "true"
    )
    OPTIONS_TICK_CAPTURE_MAX_CONTRACTS = int(
        os.environ.get("OPTIONS_TICK_CAPTURE_MAX_CONTRACTS", "40")
    )

    # ─── Volume Activity Tracker ──────────────────────────────────────────────
    # Monitora variação de volume em TODAS as opções da cadeia (incluindo sem OI).
    OPTIONS_VOLUME_TRACK_ENABLE = (
        os.environ.get("OPTIONS_VOLUME_TRACK_ENABLE", "False").lower() == "true"
    )
    OPTIONS_VOLUME_TRACKER_SERVICE_PORT = int(
        os.environ.get("OPTIONS_VOLUME_TRACKER_SERVICE_PORT", "5015")
    )
    OPTIONS_VOLUME_TRACKER_SERVICE_URL = os.environ.get(
        "OPTIONS_VOLUME_TRACKER_SERVICE_URL",
        f"http://127.0.0.1:{OPTIONS_VOLUME_TRACKER_SERVICE_PORT}",
    ).rstrip("/")
    OPTIONS_VOLUME_TRACKER_REQUEST_TIMEOUT_SECONDS = float(
        os.environ.get("OPTIONS_VOLUME_TRACKER_REQUEST_TIMEOUT_SECONDS", "120")
    )
    ...
    OPTIONS_VOLUME_ACTIVITY_LOOKBACK_DAYS = int(
        os.environ.get("OPTIONS_VOLUME_ACTIVITY_LOOKBACK_DAYS", "2")
    )
    ...
    OPTIONS_OI_BACKFILL_LOOKBACK_DAYS = int(
        os.environ.get("OPTIONS_OI_BACKFILL_LOOKBACK_DAYS", "180")
    )
    ...
    OPTIONS_DAILY_HISTORY_UPDATE_HOUR = int(
        os.environ.get("OPTIONS_DAILY_HISTORY_UPDATE_HOUR", "18")
    )
    OPTIONS_B3_DAILY_SNAPSHOT_ENABLE = (
        os.environ.get("OPTIONS_B3_DAILY_SNAPSHOT_ENABLE", "True").lower() == "true"
    )
    OPTIONS_B3_DAILY_SNAPSHOT_TIME = os.environ.get(
        "OPTIONS_B3_DAILY_SNAPSHOT_TIME", "09:00"
    ).strip()
    OPTIONS_MODEL_SCHEDULE_ENABLE = (
        os.environ.get("OPTIONS_MODEL_SCHEDULE_ENABLE", "True").lower() == "true"
    )
    ...
    OPTIONS_WYRM_AUTORUN_ENABLE = (
        os.environ.get("OPTIONS_WYRM_AUTORUN_ENABLE", "True").lower() == "true"
    )
    ...
    OPTIONS_WYRM_RETRY_COOLDOWN_SECONDS = int(
        os.environ.get("OPTIONS_WYRM_RETRY_COOLDOWN_SECONDS", "1200")
    )
    ...
    OPTIONS_MODEL_DEFAULT_TIER = (
        os.environ.get("OPTIONS_MODEL_DEFAULT_TIER", "full").strip().lower()
    )
    OPTIONS_MODEL_SIGN_CONVENTION = (
        os.environ.get("OPTIONS_MODEL_SIGN_CONVENTION", "neutral").strip().lower()
    )
    OPTIONS_MODEL_SIGNAL_THRESHOLD = float(os.environ.get("OPTIONS_MODEL_SIGNAL_THRESHOLD", "0.20"))
    OPTIONS_MODEL_CONTRACT_POINT_VALUE = float(
        os.environ.get("OPTIONS_MODEL_CONTRACT_POINT_VALUE", "1.0")
    )
    ...
    OPTIONS_MODEL_MIN_TIME_YEARS = float(
        os.environ.get("OPTIONS_MODEL_MIN_TIME_YEARS", str(1 / 2520))
    )
    ...
    OPTIONS_MODEL_SPOT_SECURITY_MAP = _parse_key_value_mapping(
        os.environ.get(
            "OPTIONS_MODEL_SPOT_SECURITY_MAP", "IBOVE Index=IBOV Index,SPX Index=SPX Index"
        )
    )
    OPTIONS_MODEL_FORWARD_SECURITY_MAP = _parse_key_value_mapping(
        os.environ.get(
            "OPTIONS_MODEL_FORWARD_SECURITY_MAP",
            "IBOVE Index=XB1 Index,SPX Index=ESA Index|ES1 Index",
        )
    )
    ...
    OPTIONS_MODEL_RATE_CURVE_POINTS = {
        ...
        for key, value in _parse_key_value_mapping(
            os.environ.get(
                "OPTIONS_MODEL_RATE_CURVE_POINTS", "21=0.135,42=0.136,63=0.137,84=0.138,126=0.139"
            )
        ).items()
    }
    ...
    OPTIONS_MODEL_DEALER_INFERENCE_ENABLE = (
        os.environ.get("OPTIONS_MODEL_DEALER_INFERENCE_ENABLE", "True").lower() == "true"
    )
    ...
    OPTIONS_MODEL_RANGE_PROJECTION_ENABLE = (
        os.environ.get("OPTIONS_MODEL_RANGE_PROJECTION_ENABLE", "True").lower() == "true"
    )
    ...
    OPTIONS_GLOBAL_TRIANGULATION_ENABLE = (
        os.environ.get("OPTIONS_GLOBAL_TRIANGULATION_ENABLE", "True").lower() == "true"
    )
    OPTIONS_GLOBAL_TRIANGULATION_ASSETS_JSON = os.environ.get(
        "OPTIONS_GLOBAL_TRIANGULATION_ASSETS_JSON", ""
    )
    ...
    OPTIONS_FAIR_VALUE_ENABLE = (
        os.environ.get("OPTIONS_FAIR_VALUE_ENABLE", "True").lower() == "true"
    )
    OPTIONS_FAIR_VALUE_LOOKBACK_HOURS = int(
        os.environ.get("OPTIONS_FAIR_VALUE_LOOKBACK_HOURS", "120")
    )
    OPTIONS_FAIR_VALUE_MAX_SNAPSHOTS = int(
        os.environ.get("OPTIONS_FAIR_VALUE_MAX_SNAPSHOTS", "1600")
    )
    ...
    OPTIONS_FAIR_VALUE_RLS_FORGETTING = float(
        os.environ.get("OPTIONS_FAIR_VALUE_RLS_FORGETTING", "0.985")
    )
    ...
    OPTIONS_FAIR_VALUE_LIVE_ENABLE = (
        os.environ.get("OPTIONS_FAIR_VALUE_LIVE_ENABLE", "False").lower() == "true"
    )
    ...
    MARKET_SCREEN_W32_OCR_SCALE = float(os.environ.get("MARKET_SCREEN_W32_OCR_SCALE", "2.0"))
    ...
    FAIR_VALUE_LEGS_LIVE_HEAD_BYTES = int(os.environ.get("FAIR_VALUE_LEGS_LIVE_HEAD_BYTES", "0"))
    ...
    OPTIONS_INTRADAY_NEURAL_MIN_ROWS = int(os.environ.get("OPTIONS_INTRADAY_NEURAL_MIN_ROWS", "80"))
    OPTIONS_INTRADAY_NEURAL_HIDDEN_WIDTH = int(
        os.environ.get("OPTIONS_INTRADAY_NEURAL_HIDDEN_WIDTH", "10")
    )
    OPTIONS_INTRADAY_NEURAL_EPOCHS = int(os.environ.get("OPTIONS_INTRADAY_NEURAL_EPOCHS", "180"))
    OPTIONS_INTRADAY_NEURAL_PATIENCE = int(os.environ.get("OPTIONS_INTRADAY_NEURAL_PATIENCE", "24"))
    ...
    OPTIONS_MODEL_DAILY_INSIGHTS_ENABLE = (
        os.environ.get("OPTIONS_MODEL_DAILY_INSIGHTS_ENABLE", "True").lower() == "true"
    )
    ...
    def validate(cls):
        ...
        if cls.AUTH_ENABLED:
            if not cls.AUTH_TOKEN_SECRET:
                errors.append(
                    "AQUILES_AUTH_TOKEN_SECRET is required when authentication is enabled"
                )
        ...
        if cls.OPTIONS_ENABLE:
            ...
            if cls.OPTIONS_MODEL_ENABLE:
                if cls.OPTIONS_MODEL_DEFAULT_TIER not in {
                    "full",
                    "structural",
                    "liquid",
                    "critical",
                    ...
                    errors.append(
                        "OPTIONS_MODEL_DEFAULT_TIER must be full, structural, liquid or critical"
                    )
                if cls.OPTIONS_MODEL_SIGN_CONVENTION not in {
                    "neutral",
                    "dealer_short_optionality",
                    "heuristic",
                }:
                    errors.append(
                        "OPTIONS_MODEL_SIGN_CONVENTION must be neutral, dealer_short_optionality or heuristic"
                    )
