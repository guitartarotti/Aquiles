...
except Exception:  # pragma: no cover - optional when running local backend
    Zep = None
...
class NodeInfo:
    ...
    def to_text(self) -> str:
        """转换为文本格式"""
        entity_type = next(
            (label for label in self.labels if label not in ["Entity", "Node"]),
            "未知类型",
        )
...
class ZepToolsService:
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        self.client = None
        if self.graph_backend == "zep_cloud":
            if not self.api_key:
                raise ValueError("ZEP_API_KEY 未配置")
            if Zep is None:
                raise ValueError("zep_cloud SDK 未安装")
            self.client = Zep(api_key=self.api_key)

        self.graph_builder = GraphBuilderService(
            api_key=self.api_key if self.graph_backend == "zep_cloud" else None,
            backend_name=self.graph_backend,
        )
        self._graph_cache: Dict[str, Dict[str, Any]] = {}
        self._last_graph_id: Optional[str] = None
        # LLM客户端用于InsightForge生成子问题
        self._llm_client = llm_client
        logger.info(f"{t('console.zepToolsInitialized')} backend={self.graph_backend}")
    ...
    def _get_graph_snapshot(self, graph_id: str, refresh: bool = False) -> Dict[str, Any]:
        if refresh or graph_id not in self._graph_cache:
            self._graph_cache[graph_id] = self.graph_builder.get_graph_data(graph_id)
        self._last_graph_id = graph_id
        return self._graph_cache[graph_id]

    def _find_cached_node(self, node_uuid: str) -> Optional[Dict[str, Any]]:
        graph_ids = []
        if self._last_graph_id:
            graph_ids.append(self._last_graph_id)
        graph_ids.extend(graph_id for graph_id in self._graph_cache if graph_id not in graph_ids)

        for graph_id in graph_ids:
            snapshot = self._graph_cache.get(graph_id) or {}
    ...
    @staticmethod
    def _to_node_info(node: Dict[str, Any]) -> NodeInfo:
        return NodeInfo(
            uuid=node.get("uuid", ""),
            name=node.get("name", ""),
    ...
    @staticmethod
    def _to_edge_info(edge: Dict[str, Any], include_temporal: bool = True) -> EdgeInfo:
        edge_info = EdgeInfo(
            uuid=edge.get("uuid", ""),
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        snapshot = self._get_graph_snapshot(graph_id)
        result = [self._to_node_info(node) for node in snapshot.get("nodes", [])]
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        snapshot = self._get_graph_snapshot(graph_id)
        result = [
            self._to_edge_info(edge, include_temporal=include_temporal)
    ...
    def get_node_detail(self, node_uuid: str, graph_id: Optional[str] = None) -> Optional[NodeInfo]:
        ...
        try:
            ...
            if graph_id:
                snapshot = self._get_graph_snapshot(graph_id)
                node_data = next(
                    (node for node in snapshot.get("nodes", []) if node.get("uuid") == node_uuid),
                    None,
                )
            else:
                node_data = self._find_cached_node(node_uuid)

            if node_data:
                return self._to_node_info(node_data)
