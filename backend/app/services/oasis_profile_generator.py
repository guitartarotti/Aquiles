...
logger = get_logger('aquiles.oasis_profile')
...
class OasisProfileGenerator:
    ...
    INDIVIDUAL_ENTITY_TYPES = [
        ...
        "hedgefundmanager", "macrotrader", "optionstrader", "portfoliomanager",
        "institutionalallocator", "retailpersonality", "sellsidestrategist",
        "treasurymanager", "quanttrader", "familyofficemanager", "proptrader",
        "eventdriventrader", "ctaoperator", "macroresearcher"
    ]
    ...
    def __init__(
        ...
        # 图谱工具用于检索丰富上下文（Zep Cloud 或 Graphiti local）
        self.graph_backend = (Config.GRAPH_BACKEND or "zep_cloud").strip().lower()
        ...
        try:
            ...
            logger.warning(f"图谱检索工具初始化失败: {e}")
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if not self.graph_tools:
            logger.debug("跳过图谱检索：graph_tools 不可用")
            return results
        ...
        try:
            ...
            all_facts = set(edge_result.facts if edge_result else [])
            ...
            if node_result:
                for node in node_result.nodes:
                    ...
                    if node_name and node_name != entity_name:
                        all_summaries.add(f"相关实体: {node_name}")
            ...
            logger.info(
                ...
                f"{len(results['node_summaries'])} 个相关节点, backend={self.graph_backend}"
            )
        except Exception as e:
...
- 机构账号发言要符合其身份定位"""
    ...
    def _generate_profile_rule_based(
        ...
        elif entity_attributes.get("macro_persona") or entity_type_lower in [
            "hedgefundmanager", "macrotrader", "optionstrader", "portfoliomanager",
            "institutionalallocator", "retailpersonality", "sellsidestrategist",
            "treasurymanager", "quanttrader", "familyofficemanager", "proptrader",
            "eventdriventrader", "ctaoperator", "macroresearcher"
        ]:
            institution = entity_attributes.get("institution", "Independent desk")
            desk = entity_attributes.get("desk", "Macro")
            strategy = entity_attributes.get(
                "strategy",
                entity_summary or "Interprets market context and builds scenarios."
            )
            time_horizon = entity_attributes.get("time_horizon", "intraday to multi-day")
            risk_style = entity_attributes.get("risk_style", "balanced")
            communication_style = entity_attributes.get("communication_style", "clear and market-focused")
            signature_move = entity_attributes.get(
                "signature_move",
                "Uses cross-asset confirmation before acting."
            )
            watch_signals = entity_attributes.get("watch_signals", [])
            interested_topics = entity_attributes.get("interested_topics", [])

            if not interested_topics:
                interested_topics = ["Macro", "Rates", "FX", "Equities"]

            profession = entity_attributes.get("profession") or entity_type
            primary_signals = ", ".join(watch_signals[:4]) if watch_signals else "rates, FX, equities and headlines"

            return {
                "bio": f"{profession} at {institution} | desk={desk} | horizon={time_horizon}",
                "persona": (
                    f"{entity_name} operates as a {profession} focused on macro regime interpretation. "
                    f"Their core strategy is {strategy} They communicate in a {communication_style} style, "
                    f"carry a {risk_style} risk profile, and are known for the signature move: {signature_move} "
                    f"Primary watch signals include {primary_signals}."
                ),
                "age": random.randint(29, 57),
                "gender": random.choice(["male", "female"]),
                "mbti": random.choice(["ENTJ", "INTJ", "ENTP", "ESTJ", "ISTJ"]),
                "country": entity_attributes.get("country", "Brazil"),
                "profession": profession,
                "interested_topics": interested_topics,
                "friend_count": random.randint(120, 900),
                "follower_count": random.randint(800, 12000),
                "statuses_count": random.randint(400, 6000),
                "karma": random.randint(700, 9000),
            }
