...
from .macro_persona_service import MACRO_PERSONA_ENTITY_TYPES
from .zep_entity_reader import EntityNode

logger = get_logger('aquiles.simulation_config')
...
class SimulationConfigGenerator:
    ...
    def _is_macro_persona_run(self, entities: List[EntityNode]) -> bool:
        entity_types = {
            (entity.get_entity_type() or "").strip()
            for entity in entities
            if entity.get_entity_type()
        }
        return any(entity_type in MACRO_PERSONA_ENTITY_TYPES for entity_type in entity_types)

    def _build_macro_type_info(self, entities: List[EntityNode]) -> str:
        type_examples = {}
        for entity in entities:
...
- reasoning (string): 简要说明为什么这样配置"""
    ...
    def _generate_event_config(
        ...
        is_macro_run = self._is_macro_persona_run(entities)
        type_info = self._build_macro_type_info(entities)
        
        # 使用配置的上下文截断长度
        context_truncated = context[:self.EVENT_CONFIG_CONTEXT_LENGTH]
        
        if is_macro_run:
            prompt = f"""基于以下模拟需求，生成宏观市场讨论事件配置。
...
## 任务
请生成适合市场讨论 playground 的事件配置 JSON：
- 热点主题应围绕利率曲线、指数、美元、资金流、政策、风险偏好
- narrative_direction 要描述市场接下来会围绕什么争论
- initial_posts 中每条帖子都必须指定 poster_type
- poster_type 必须从上面的可用角色类型中选择，不能使用 Contract/Broker/Security/News 这类数据节点类型
- 内容应体现“谁先发声、谁质疑、谁做对冲、谁解释给市场听”

返回JSON格式（不要markdown）：
{{
    "hot_topics": ["关键词1", "关键词2", "..."],
    "narrative_direction": "<市场叙事方向>",
    "initial_posts": [
        {{"content": "帖子内容", "poster_type": "HedgeFundManager"}},
        {{"content": "帖子内容", "poster_type": "MacroTrader"}}
    ],
    "reasoning": "<简要说明>"
}}"""

            system_prompt = (
                "你是巴西宏观市场与交易台行为分析专家。返回纯JSON格式。"
                " poster_type 必须精确匹配可用的角色类型，且只能是角色，不能是市场数据节点类型。"
            )
        else:
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        type_aliases = {
            ...
            "hedgefundmanager": ["hedgefundmanager", "macrotrader", "portfoliomanager"],
            "macrotrader": ["macrotrader", "hedgefundmanager", "proptrader", "eventdriventrader"],
            "optionstrader": ["optionstrader", "macrotrader", "proptrader"],
            "portfoliomanager": ["portfoliomanager", "hedgefundmanager", "institutionalallocator", "familyofficemanager"],
            "institutionalallocator": ["institutionalallocator", "portfoliomanager", "familyofficemanager"],
            "retailpersonality": ["retailpersonality", "macroresearcher"],
            "sellsidestrategist": ["sellsidestrategist", "macroresearcher"],
            "treasurymanager": ["treasurymanager", "institutionalallocator"],
            "quanttrader": ["quanttrader", "ctaoperator", "proptrader"],
            "familyofficemanager": ["familyofficemanager", "portfoliomanager", "institutionalallocator"],
            "proptrader": ["proptrader", "macrotrader", "optionstrader"],
            "eventdriventrader": ["eventdriventrader", "macrotrader", "hedgefundmanager"],
            "ctaoperator": ["ctaoperator", "quanttrader"],
            "macroresearcher": ["macroresearcher", "sellsidestrategist", "retailpersonality"],
        }
    ...
    def _generate_agent_configs_batch(
        ...
        if is_macro_run:
            prompt = f"""基于以下信息，为每个宏观市场角色生成讨论行为配置。
...
为每个角色生成活动配置，注意：
- 这是市场讨论场景，不是校园/舆情场景
- 活跃时间应更贴近巴西市场节奏：盘前准备、日间交易、收盘后复盘
- HedgeFundManager / MacroTrader / PropTrader / EventDrivenTrader：白天高活跃，响应极快，影响力高
- OptionsTrader / QuantTrader / CTAOperator：事件窗口和波动扩大时更活跃
- PortfolioManager / InstitutionalAllocator / FamilyOfficeManager / TreasuryManager：发言频率低，但影响力和权重高
- RetailPersonality / MacroResearcher / SellSideStrategist：负责放大、解释、框定叙事
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<0-23小时列表，贴近BRT交易节奏>],
            "response_delay_min": <最小响应分钟>,
            "response_delay_max": <最大响应分钟>,
            "sentiment_bias": <-1.0到1.0>,
            "stance": "<supportive/opposing/neutral/observer>",
            "influence_weight": <影响力>
        }}
    ]
}}"""

            system_prompt = "你是宏观交易台与市场微观行为分析专家。返回纯JSON。"
        else:
