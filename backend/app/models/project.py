...
class Project:
    ...
    project_mode: str = "document"
    agent_strategy: str = "graph_entities"
    chunk_size: int = 500
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Project':
        ...
        return cls(
            ...
            project_mode=data.get('project_mode', 'document'),
            agent_strategy=data.get('agent_strategy', 'graph_entities'),
...
class ProjectManager:
    ...
    def find_project_by_graph_id(cls, graph_id: str) -> Optional[Project]:
        """Find the project that owns a given graph_id."""
        if not graph_id:
            return None
    ...
    @classmethod
    def save_text_file_to_project(cls, project_id: str, original_filename: str, content: str) -> Dict[str, str]:
        """Save generated text content into the project files directory."""
        files_dir = cls._get_project_files_dir(project_id)
        os.makedirs(files_dir, exist_ok=True)

        ext = os.path.splitext(original_filename)[1].lower() or '.md'
        safe_filename = f"{uuid.uuid4().hex[:8]}{ext}"
        file_path = os.path.join(files_dir, safe_filename)
