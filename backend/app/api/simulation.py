...
logger = get_logger('aquiles.api.simulation')
...
def ensure_graph_backend_ready():
    """Validate whichever graph backend is active for this deployment."""
    try:
        GraphBuilderService()
        return None
    except Exception as exc:
        logger.exception("graph backend validation failed", exc_info=exc)
        return True
    ...
    请求（JSON）：
        {
            ...
            "graph_id": "aquiles_xxxx",    // 可选，如不提供则从project获取
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        logger.exception("simulation preparation status read failed", exc_info=e)
        return False, {"reason": "Simulation preparation status is unavailable"}
...
def prepare_simulation():
    ...
    try:
        ...
        try:
            ...
            resolver = SimulationEntityResolver()
            # 快速读取实体（不需要边信息，只统计数量）
            filtered_preview = resolver.resolve_filtered_entities(
        ...
        def run_prepare():
            ...
            try:
                ...
                task_manager.fail_task(task_id, "Internal server error")
        ...
        return error_response(logger, status_code=404, message='Simulation not found or not ready')
...
def generate_profiles():
    ...
    try:
        ...
        resolver = SimulationEntityResolver()
        filtered = resolver.resolve_filtered_entities(
            graph_id=graph_id,
            project_id=data.get('project_id'),
            defined_entity_types=entity_types,
...
def start_simulation():
    ...
    try:
        ...
        return error_response(logger, status_code=400, message='Invalid simulation state')
...
def interview_agent():
    ...
    try:
        ...
        return error_response(logger, status_code=400, message='Invalid interview request')
        
    except TimeoutError as e:
        return error_response(logger, status_code=504, exception=e)
...
def close_simulation_env():
    ...
    try:
        ...
        return error_response(logger, status_code=400, message='Invalid simulation environment request')
